...
speakEL.addEventListener('click', () => {
    ...
    // Track the event when speech starts and stops
    utterance.onstart = () => {
    ...
    utterance.onend = () => {
        if (audioBlob) {
            ...
            downloadEL.href = url;
            downloadEL.download = 'spoken-text.wav';
        }
    };
...
// A dictionary (object) to store questions and answers
...
// Toggle Chatbox visibility with smooth animation
chatbotButton.addEventListener('click', (event) => {
    event.stopPropagation(); // Prevent the click event from propagating to the document
    if (chatbox.style.display === 'none' || chatbox.classList.contains('hide')) {
        ...
        setTimeout(() => chatbox.style.display = 'none', 300); // Wait for animation to finish
    }
});

// Handle sending messages
sendMessageButton.addEventListener('click', () => {
    const userMessage = userInput.value.trim().toLowerCase(); // Convert message to lowercase
    if (userMessage) {
        addMessage('ME', userInput.value);
        userInput.value = '';

        // Find the reply from the chatbotResponses object
...
function addMessage(sender, text) {
    ...
    if (sender === 'ME') {
        ...
        aiIcon.classList.add('fas', 'fa-robot'); // FontAwesome icon for AI (robot icon)
